...
excel_file = os.path.join(script_dir, '..', '東京3_26 - 3_31.xlsx')

# 如果上面的路徑不存在，嘗試其他路徑
if not os.path.exists(excel_file):
    excel_file = os.path.join(os.path.dirname(script_dir), '東京3_26 - 3_31.xlsx')

print(f"嘗試讀取: {excel_file}")
    ...
    for col in range(1, 6):
        for row in range(1, min(200, sheet.max_row + 1)):
            ...
            if url and url not in seen_urls:
                ...
                elif not any(k in value for k in ['Day', '時間', '行程', '活動', 'Cost', 'JPY', 'TWD', '預估', 'Time']):
                    if value not in notes_lines:
                        notes_lines.append(value)
    
    notes_content = '\n'.join(notes_lines)
    
    print(f"\n找到 {len(seen_urls)} 個連結")
    print(f"備註內容:\n{notes_content}")
    
    # 保存到文件
    output_file = os.path.join(script_dir, 'extracted_notes.txt')
